...
class ReportModelView(BaseView):
    ...
    def generate_report(self, start_date, end_date, status, nome, email, setor, patrimonio):
        try:
            ...
            query_filters = [
                (Ticket.horario_inicial_atendimento >= start_date) | (Ticket.horario_final_atendimento >= start_date),
                (Ticket.horario_inicial_atendimento <= end_date) | (Ticket.horario_final_atendimento <= end_date)
            ]

            if status:
                query_filters.append(Ticket.status == status)

            if nome:
                query_filters.append(Ticket.nome == nome)

            if email:
                query_filters.append(Ticket.email == email)

            if setor:
                query_filters.append(Ticket.setor == setor)

            if patrimonio:
                query_filters.append(Ticket.patrimonio == patrimonio)

            relevant_tickets = Ticket.query.filter(and_(*query_filters)).all()
